...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    checkResumeExam();
    displayRandomQuote();
    updateStartButtonIcon();   // تحديث أيقونة زر البداية حسب الثيم الحالي
});
...
function selectOption(optionIndex) {
    ...
    if (mode === 'training') {
        ...
        if (isCorrect) {
            ...
            showCelebration();   // استدعاء الاحتفالية حسب الثيم
            saveExamState();
            renderExam();
        } else {
            renderExam();
...
function toggleGrid() {
    ...
    if (grid.classList.contains('hidden')) {
        btn.innerHTML = '<span>☰</span> Show Grid';
    } else {
        btn.innerHTML = '<span>☰</span> Hide Grid';
    }
}
...
function showResults() {
    ...
    if (score > 50) {
        showCelebration();   // احتفالية عند النجاح
    }
...
// ============================================
// STATISTICS LOGIC (Updated with two sections)
// ============================================
...
function renderStatistics() {
    ...
    // قائمة السنوات مع عدد الأسئلة والمحلول
    let totalYearQuestions = 0;
    let totalYearAnswered = 0;
    allYears.forEach(year => {
        ...
        totalYearQuestions += total;
        totalYearAnswered += answered;
    ...
    `;
    html += '</div>'; // نهاية قسم السنوات

    // ========== قسم المحاضرات (اليسار) ==========
    html += '<div style="flex: 1; min-width: 280px; border: 1px solid var(--border); border-radius: var(--radius); padding: 16px;">';
    html += '<h4 style="color: var(--primary); margin-bottom: 16px;">📚 Lectures Statistics</h4>';

    let totalLectureQuestions = 0;
    let totalLectureAnswered = 0;
    allLectures.forEach(lecture => {
        const key = `lecture-${lecture.name}`;
        const prog = progress[key] || { questionIds: [] };
        const total = lecture.questions.length;
        const answered = prog.questionIds ? prog.questionIds.length : 0;
        totalLectureQuestions += total;
        totalLectureAnswered += answered;
    ...
    `;
    html += '</div>'; // نهاية قسم المحاضرات

    html += '</div>'; // نهاية الحاوية المرنة
...
function changeTheme(theme) {
    ...
    saveSettings();
    // تحديث أيقونة زر البداية حسب الثيم
...
function showCelebration() {
    ...
    switch (theme) {
        ...
        case 'desert':
            ...
            break;
        default:
            showFireworks();   // default fireworks
    }
}
...
function ghostCelebration() {
    ...
    function drawGhost(x, y, size) {
        ...
        for (let i = 0; i < 3; i++) {
            const wave = size/4;
            ctx.quadraticCurveTo(-size/2 + wave + i*wave, wave/2, -size/2 + wave*2*i, 0);
        }
        ctx.lineTo(size/2, 0);
        ctx.closePath();
        ctx.fill();
        ctx.fillStyle = '#000';
        ctx.beginPath();
        ctx.arc(-size/5, -size/3, size/8, 0, Math.PI*2);
        ctx.arc(size/5, -size/3, size/8, 0, Math.PI*2);
        ctx.fill();
        ctx.restore();
    }
...
function desertCelebration() {
    ...
    for (let i = 0; i < 60; i++) {
        sands.push({
            ...
            color: `rgba(210, 180, 140, ${Math.random() * 0.7 + 0.3})`
        });
    }

    let frame = 0;
    function animate() {
        ctx.fillStyle = 'rgba(253, 245, 230, 0.15)';
        ctx.fillRect(0, 0, canvas.width, canvas.height);
        sands.forEach(s => {
            s.x += s.vx;
            s.y += s.vy;
