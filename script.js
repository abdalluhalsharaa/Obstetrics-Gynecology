...
function renderStatistics() {
    ...
    allLectures.forEach(lecture => {
        ...
        const prog = progress[key] || { answered: 0, correct: 0 };
        const total = lecture.questions.length;
        const pct = total > 0 ? Math.min(100, Math.round((prog.answered / total) * 100)) : 0;
    ...
    allYears.forEach(year => {
        ...
        const pct = total > 0 ? Math.round((prog.answered / total) * 100) : 0;
...
function saveProgress() {
    ...
    questions.forEach((q, i) => {
        if (firstAnswers[i] !== null) {
            ...
            if (!progress[key]) progress[key] = { answered: 0, correct: 0 };
            progress[key].answered++;
            if (isAnswerCorrect(q, firstAnswers[i])) {
